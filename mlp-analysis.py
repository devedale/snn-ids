...
class MLPAnalysisBenchmark:
    ...
    def _run_single_test(self, X: np.ndarray, y: np.ndarray, label_encoder: Any, hyperparams: Dict, run_id: str) -> Dict:
        ...
        if np.min(class_counts) < 2:
            print("⚠️ Warning: One or more classes have only 1 sample. Disabling stratification for this run.")
            stratify_opt = None
        else:
            stratify_opt = y
    ...
    def _generate_final_report(self, all_results: List[Dict], best_run: Dict, total_time: float) -> Dict:
        ...
        return {
            ...
            'best_configuration': {
                ...
                'per_class_loss_history': {int(k): v for k, v in best_run['per_class_loss_history'].items()} if best_run.get('per_class_loss_history') else None
            },
