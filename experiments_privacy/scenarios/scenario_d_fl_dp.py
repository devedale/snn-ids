...
def run(
    ...
    res = simulate_fl_pipeline(cfg, builder_kwargs=builder_kwargs)
    # stima epsilon usando dimensioni del dataset ricavate indirettamente
    # NB: qui non abbiamo dataset_size esplicito; assumiamo proxy da batch*clients*local_epochs
    steps = rounds * num_clients * local_epochs
    dataset_size_proxy = max(10000, batch_size * num_clients * 100)  # proxy conservativo
